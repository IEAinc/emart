...
// 엑셀 파일 다운로드 함수
    async function downloadExcel(data, filename = 'data_with_images.xlsx') {
        try {
            console.log('엑셀 파일 생성 중...');
            const workbook = await createExcelWithImages(gridData);
  ...
  const searchData=async (data)=>{
    ...
    const MakeExcel=()=>{
        let data=gridData.map(({generateImgPreview,productImg,...etc})=>{
            console.log(generateImgPreview.imgList[0].img)
            console.log(productImg.imgList[0].img)
            return{
